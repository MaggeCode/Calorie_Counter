...
# Create a dashboard with all graphs shown at the same time using the gridExtra package     
  require(gridExtra)
...
#Zu readme hinzufügen:  "Open plot in zoom mode for proper displaying of the results" "Install required packages : library(ggplot2) library(scales) library(dplyr) library(gridExtra)"
  
  
  
  
  #ideas for charts
  #scatter plot oncalories per ages --> + maybe trend line
  
  # order history and trends
  
  
  # per age how much percent of daily calorie input do they consume in one order

#kommentar kieron fürs speichern: 3 Github ordner. Jeweils einer mit Code von einer Person. Dann ein ordner "exchange" wo der erste
#code die daten erstellt und speichert in einer csv datei. am ende wid mein code ausgeführt, der greift die datei aus dem ordner ab.
#Wichtig: Zusätzlich sample datei mit liefern --> also eventuell zweite "code-datei" die einen sample datensatz ausführt, um die
# ergebnisse vernünfitg zu zeigen 
